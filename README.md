...
신한은행 거래내역조회 PDF, 김포페이(지역화폐) 거래내역서 xlsx, 수동 전사 CSV(앱 캡처·종이 명세서만 있는 은행용, 예: 기업은행).

김포페이는 결제를 거래금액(가맹점 가격) 기준 지출로, 충전은 이체(은행 출금과 대응)로 넣고, 충전 시 붙는 할인형
인센티브와 결제에 쓰인 기후행동기회소득·걷기챌린지 지원금은 `수입 > 환급/정산`으로 잡습니다.
